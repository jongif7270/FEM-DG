function [error,rate]=FEMDGerror(t,N)
...
h=1./Ma;
for i =1:iter
    [c4n,n4e,ind4e,~,~,~,~,~] = mesh_FEMDG(xl,xr,yl,yr,Ma(i),Ma(i),N);
    [u,V,Dr,Ds,~] = FEMDG(Ma(i),N);
    error(i)=compute_error_fem_2d_triangle(c4n,n4e,ind4e,V,Dr,Ds,u,ux,uy,N);
end
%plot(time,error)
% loglog(2.^time,error)
