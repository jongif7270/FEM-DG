function [u,V2D,Dr,Ds,c4n2] = HDG8(M,N)
%% 
...
%% Ex 3.4
xl=-1;xr=1;yl=-1;yr=1;Mx=M;My=M;    a=@(x) [exp(x(:,1)).*(x(:,2).*cos(x(:,2))+sin(x(:,2))), -exp(x(:,1)).*x(:,2).*sin(x(:,2))]; b=0;e=0.1;S=-1;k=1; f=@(x) b*sin(pi*x(:,1)).*sin(pi*x(:,2)) +(exp(x(:,1)).*(x(:,2).*cos(x(:,2))+sin(x(:,2)))).*pi.*cos(pi*x(:,1)).*sin(pi*x(:,2)) +(-exp(x(:,1)).*x(:,2).*sin(x(:,2))).*pi.*sin(pi*x(:,1)).*cos(pi*x(:,2)) +e*2*pi^2*sin(pi*x(:,1)).*sin(pi*x(:,2)); ue=@(x) sin(pi*x(:,1)).*sin(pi*x(:,2));
...
[ic4n,in4e,~,~] = mesh_fem_2d_triangle(xl,xr,yl,yr,Mx,My,int*N);
[~,~,~,ic4n2,~] = indexforDG(xl,xr,yl,yr,Mx,My,int*N);
...
for j=1:size(n4e,1)
    ...
    for i=1:size(in4e,2)
        n=normal(ic4n(in4e(j,mod(i,3)+1),:)-ic4n(in4e(j,mod(i-1,3)+1),:));
        h=norm(ic4n(in4e(j,mod(i,3)+1),:)-ic4n(in4e(j,mod(i-1,3)+1),:));
        ...
        if vp>vn
            ...
            if e4s(s4e(j,i),2)==j
                Ba(:,(i-1)*(N+1)+(N+1:-1:1))=S*e*(h/2)*((rx(j)*Dr(en(s4e(j,i),:,1),:)'*M+sx(j)*Ds(en(s4e(j,i),:,1),:)'*M)*n(1)+(ry(j)*Dr(en(s4e(j,i),:,1),:)'*M+sy(j)*Ds(en(s4e(j,i),:,1),:)'*M)*n(2));
                Bb(en(s4e(j,i),:,1),(i-1)*(N+1)+(N+1:-1:1))=Bb(en(s4e(j,i),:,1),(i-1)*(N+1)+(N+1:-1:1))-e*(h/2)*(k/ht)*M+(h/2)*A1D*iM.*v'*A1D';
